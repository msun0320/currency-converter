...
const Converter = () => {
	...
	const ACCESS_KEY = "f8aab0df2726b581392b3c11d76efea2";
	const BASE_URL = "http://data.fixer.io/api/";

	useEffect(() => {
		async function fetchData() {
			const currencyResponse = await fetch(`${BASE_URL}/latest?access_key=${ACCESS_KEY}&symbols=${fromCurrency},${toCurrency}`);
			const currencyData = await currencyResponse.json();
			const rates = currencyData.rates;

			setToAmount(fromAmount / rates[fromCurrency] * rates[toCurrency]);
		}
		fetchData();
	...
	return (
		<>
			<div>
				...
				<select name="from" id="from" defaultValue="USD" onChange={handleFromCurrencyChange}>
			...
			<div>
				...
				<select name="to" id="to" defaultValue="EUR" onChange={handleToCurrencyChange}>
			    <option value="AED">United Arab Emirates dirham</option>
			    <option value="AFN">Afghan afghani</option>
			    <option value="ALL">Albanian lek</option>
			    <option value="AMD">Armenian dram</option>
				...
				<p className="to-amount">{Math.round(toAmount * 100) / 100}</p>
			</div>
		</>
	);
};
